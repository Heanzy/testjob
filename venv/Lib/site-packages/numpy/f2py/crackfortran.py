...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        if case in ['call', 'callfun']:  # set parents variables
            ...
            groupcache[groupcounter]['vars'] = copyAction.deepcopy(
    ...
    for e in el1:
        ...
        if ename in groupcache[groupcounter]['vars']:
            ...
            if 'kindselector' not in edecl:
                edecl['kindselector'] = copyAction.copy(kindselect)
            elif kindselect:
                for k in list(kindselect.keys()):
                    if k in edecl['kindselector'] and (not kindselect[k] == edecl['kindselector'][k]):
                        ...
                        edecl['kindselector'][k] = copyAction.copy(kindselect[k])
            if 'charselector' not in edecl and charselect:
                ...
                for k in list(charselect.keys()):
                    if k in edecl['charselector'] and (not charselect[k] == edecl['charselector'][k]):
                        ...
                        edecl['charselector'][k] = copyAction.copy(charselect[k])
            ...
            if 'attrspec' not in edecl:
                edecl['attrspec'] = copyAction.copy(attrspec)
            elif attrspec:
                for a in attrspec:
                    if a not in edecl['attrspec']:
                        edecl['attrspec'].append(a)
        else:
            edecl['typespec'] = copyAction.copy(typespec)
            edecl['kindselector'] = copyAction.copy(kindselect)
            edecl['charselector'] = copyAction.copy(charselect)
            edecl['typename'] = typename
            edecl['attrspec'] = copyAction.copy(attrspec)
        if m.group('after'):
...
def postcrack(block, args=None, tab=''):
    ...
    if 'externals' in block and block['externals']:
        ...
        mvars = copyAction.copy(block['vars'])
        if name:
...
def get_parameters(vars, global_params={}):
    params = copyAction.copy(global_params)
    g_params = copyAction.copy(global_params)
...
def analyzevars(block):
    ...
    vars = copyAction.copy(block['vars'])
    ...
    if not block['block'] in ['module', 'pythonmodule', 'python module', 'block data']:
        if 'commonvars' in block:
            neededvars = copyAction.copy(block['args'] + block['commonvars'])
        else:
            neededvars = copyAction.copy(block['args'])
        for n in list(vars.keys()):
